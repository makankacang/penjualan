...
--
-- Host: localhost:3306
-- Generation Time: Feb 29, 2024 at 03:55 PM
...
INSERT INTO `orders` (`id`, `kode_order`, `pembayaran_id`, `transaksi_detail_id`, `transaksi_id`, `pelanggan_id`, `created_at`, `updated_at`) VALUES
(12, 'O001', 12, 78, 25, 1, '2024-02-29 07:26:30', '2024-02-29 07:26:30');
...
INSERT INTO `pembayaran` (`pembayaran_id`, `waktu_bayar`, `total`, `metode`, `transaksi_id`, `no_rek`) VALUES
(12, '2024-02-29 07:26:28', 110000, 'TUNAI', 25, NULL);
...
(25, '2024-02-29 12:55:24', 'jhjhjh', 100000, 1, 'pending');
...
--
DELIMITER $$
CREATE TRIGGER `update_transaksi_total` AFTER INSERT ON `transaksi_detail` FOR EACH ROW BEGIN
    DECLARE trans_id INT;
    DECLARE total DECIMAL(10, 2);
    
    -- Get the transaksi_id of the inserted transaksi_detail
    SET trans_id = NEW.transaksi_id;

    -- Calculate the total based on the inserted/updated transaksi_detail
    SELECT SUM(harga * qty) INTO total FROM transaksi_detail WHERE transaksi_id = trans_id;
    
    -- Update the total in the transaksi table
    UPDATE transaksi SET total = total WHERE transaksi_id = trans_id;
END
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `update_transaksi_total_after_delete` AFTER DELETE ON `transaksi_detail` FOR EACH ROW BEGIN
    ...
    -- Get the transaksi_id of the deleted transaksi_detail
    SET trans_id = OLD.transaksi_id;

    -- Calculate the total based on the remaining transaksi_detail
    SELECT SUM(harga * qty) INTO total FROM transaksi_detail WHERE transaksi_id = trans_id;
...
CREATE TRIGGER `update_transaksi_total_after_update` AFTER UPDATE ON `transaksi_detail` FOR EACH ROW BEGIN
    ...
    -- Get the transaksi_id of the updated transaksi_detail
    SET trans_id = NEW.transaksi_id;

    -- Calculate the total based on the updated transaksi_detail
    SELECT SUM(harga * qty) INTO total FROM transaksi_detail WHERE transaksi_id = trans_id;
...
ALTER TABLE `orders`
  ...
  ADD KEY `transaksi_id` (`transaksi_detail_id`),
...
--
ALTER TABLE `pembayaran`
  MODIFY `pembayaran_id` int NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=13;
...
--
ALTER TABLE `transaksi`
  MODIFY `transaksi_id` int NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=26;

--
-- AUTO_INCREMENT for table `transaksi_detail`
--
ALTER TABLE `transaksi_detail`
  MODIFY `transaksi_detail_id` int NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=79;
...
--
ALTER TABLE `orders`
  ADD CONSTRAINT `orders_ibfk_1` FOREIGN KEY (`pelanggan_id`) REFERENCES `users` (`id`) ON DELETE RESTRICT ON UPDATE RESTRICT,
  ADD CONSTRAINT `orders_ibfk_2` FOREIGN KEY (`pembayaran_id`) REFERENCES `pembayaran` (`pembayaran_id`) ON DELETE RESTRICT ON UPDATE RESTRICT,
  ADD CONSTRAINT `orders_ibfk_3` FOREIGN KEY (`transaksi_detail_id`) REFERENCES `transaksi_detail` (`transaksi_detail_id`) ON DELETE RESTRICT ON UPDATE RESTRICT,
  ADD CONSTRAINT `orders_ibfk_4` FOREIGN KEY (`transaksi_id`) REFERENCES `transaksi` (`transaksi_id`) ON DELETE RESTRICT ON UPDATE RESTRICT;
